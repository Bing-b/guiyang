...
const displayImage = computed(() => {
  ...
  for (const path in images) {
    ...
    if (
      loc.includes(filename) ||
      filename.includes(loc) ||
      desc.includes(filename)
    ) {
...
const highlightText = (text: string) => {
  const transportRegex =
    /(地铁.*?号线|地铁|公交车|打车|高铁|S1 线城际|索道|步行)/g;
  const timeRegex =
    /(\d{1,2}:\d{2}|\d+(?:\.\d+)?(?:min|分钟|h|小时|～\d+h|元\/人))/g;
  const actionRegex = /(入住|早餐|午餐|晚餐|宵夜|方案 ?[A-C]?)/g;
  const poiRegex =
    /(南门进|南门出|东门进|南门|东门|弘福寺|猕猴步道|熊猫馆|动物园|瞰筑亭|黔灵湖|麒麟洞|中山西路站 F 口|小孟工业园|大十字|国际生态会议中心|灵长猴区|文昌阁|电台街|虎门巷|主景区|东山寺)/g;

  let formatted = text;
  formatted = formatted.replace(
    transportRegex,
    '<span class="text-[#007AFF] font-black">$1</span>',
  );
  formatted = formatted.replace(
    timeRegex,
    '<span class="text-[#FF4500] font-black">$1</span>',
  );
  formatted = formatted.replace(
    poiRegex,
    '<span class="text-[#42B029] font-black">$1</span>',
  );
  formatted = formatted.replace(
    actionRegex,
    '<span class="text-[#9932CC] font-black">$1</span>',
  );
...
<template>
  <div class="relative w-full flex flex-col items-center">
    ...
    <div class="mt-12 mb-4 px-2 text-center w-full max-w-[300px]">
      ...
      >
        <div
          class="absolute -top-3 left-1/2 -translate-x-1/2 text-[#F69022] bg-[#FDF9EC] px-2 font-black text-[13px] whitespace-nowrap"
        >
          🕒 {{ time }}
        </div>
        <p
          class="text-[#333] font-bold text-[15px] leading-[1.7] text-left"
    ...
    >
      ...
      <div class="space-y-6 flex-1 overflow-y-auto mb-6 pt-4 px-1">
        <div
          ...
          :key="idx"
          class="bg-white/80 p-4 pt-5 rounded-2xl border-2 border-[#42B029]/30 shadow-sm relative"
        >
          <div
            class="absolute -top-3 left-4 bg-[#42B029] text-white px-3 py-0.5 rounded-md text-[13px] font-bold shadow-sm whitespace-nowrap"
          >
            {{ opt.title }}
          </div>
          <p
            class="text-[15px] font-bold text-[#333] leading-[1.7]"
            v-html="'· ' + highlightText(opt.detail)"
          ></p>
        </div>
      </div>
